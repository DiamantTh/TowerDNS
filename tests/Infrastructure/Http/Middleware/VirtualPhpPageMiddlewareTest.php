...
final class VirtualPhpPageMiddlewareTest extends TestCase
{
    ...
    public static function paths(): iterable
    {
        ...
        yield 'forged entry' => ['/roles.php?entry=admin&id=role-42', '', 400];
        yield 'duplicate entry' => ['/roles.php?entry=roles&entry=admin', '', 400];
        yield 'direct index entry' => ['/index.php?entry=roles', '', 400];
    }
    ...
    public function testMapsOnlyKnownPages(string $uri, string $expectedPath, int $status, string $method = 'POST'): void
    {
        ...
        if (preg_match('~^/(login|admin|users|roles|accounts|zones|records|dnssec|profile|settings)\.php$~', $request->getUri()->getPath(), $matches) === 1) {
            $query['entry'] = $matches[1];
        }
        $request  = $request->withQueryParams($query)->withMethod($method)->withParsedBody(['csrf_token' => 'test']);
        $response = new VirtualPhpPageMiddleware()->process($request, $next);
        ...
        if ($status === 204) {
            ...
            self::assertArrayNotHasKey('entry', $next->seen->getQueryParams());
        }
    }
    ...
    public function testVirtualPageRequiresMatchingServerHint(): void
    {
        ...
        $request = new ServerRequest()->withUri(new Uri('/roles.php'));
        self::assertSame(400, new VirtualPhpPageMiddleware()->process($request, $next)->getStatusCode());
        self::assertSame(400, new VirtualPhpPageMiddleware()->process($request->withQueryParams(['entry' => 'users']), $next)->getStatusCode());
    }
    ...
    public function testUnsupportedMethodReturns405BeforeMezzioDispatch(): void
    {
        ...
        $request = new ServerRequest(
            ['REQUEST_URI' => '/roles.php', 'QUERY_STRING' => 'entry=roles'],
            [],
            new Uri('/roles.php?entry=roles'),
        );
        $request = $request->withQueryParams(['entry' => 'roles'])->withMethod('PUT');

        $response = new VirtualPhpPageMiddleware()->process($request, $next);

        self::assertSame(405, $response->getStatusCode());
        self::assertSame('GET, HEAD, POST, OPTIONS', $response->getHeaderLine('Allow'));
    }

    public function testApacheRewriteRetainsOnlyTheBrowserQueryForMezzio(): void
    {
        $next = new class implements RequestHandlerInterface {
            public ?ServerRequestInterface $seen = null;
        ...
        $request = new ServerRequest(
            ['REQUEST_URI' => '/roles.php?id=role-42', 'QUERY_STRING' => 'entry=roles&id=role-42'],
            [],
            new Uri('/roles.php?entry=roles&id=role-42'),
        );
        $request = $request->withQueryParams(['entry' => 'roles', 'id' => 'role-42']);

        self::assertSame(204, new VirtualPhpPageMiddleware()->process($request, $next)->getStatusCode());
        $seen = $next->seen;
        self::assertInstanceOf(ServerRequestInterface::class, $seen);
        self::assertSame('/roles/role-42', $seen->getUri()->getPath());
        self::assertSame('id=role-42', $seen->getUri()->getQuery());
        self::assertSame(['id' => 'role-42'], $seen->getQueryParams());
    }
}
