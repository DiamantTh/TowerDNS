...
final class ManagedZoneDNSServiceTest extends TestCase
{
    public function testRecordOperationResolvesOnlyTheManagedZoneProviderReference(): void
    {
        ...
        $provider = new class implements DNSProviderInterface {
            ...
            public function capabilities(): ProviderCapabilitySet { return new ProviderCapabilitySet([Capability::RECORD_CREATE => true]); }
            public function listZones(): array { return []; }
            public function createZone(string $zoneName): Zone { return new Zone('', $zoneName, 'fake', true); }
            public function deleteZone(string $zoneId): void {}
            public function listRecords(string $zoneId): array { return []; }
            public function createRecord(Record $record): Record { return $this->created = $record; }
            public function updateRecord(Record $record): Record { return $record; }
            public function deleteRecord(string $zoneId, string $recordId): void {}
            public function getDnssecProfile(string $zoneId): DNSSECProfile { return new DNSSECProfile($zoneId, DNSSECState::UNKNOWN); }
            public function executeDnssecAction(string $zoneId, string $action, array $payload = []): DNSSECProfile { return new DNSSECProfile($zoneId, DNSSECState::UNKNOWN); }
        };
