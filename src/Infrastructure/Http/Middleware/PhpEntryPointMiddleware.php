...
/** Maps physical PHP page paths to the existing Mezzio routes, before routing and profile guards. */
final readonly class PhpEntryPointMiddleware implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if ($this->hasDuplicateResourceParameter($request->getUri()->getQuery())) {
        ...
        // Body, cookies, session and original query remain untouched. RouteMiddleware
        // sees only the canonical path and applies the same route middleware/handler.
        return $this->normalizeRedirect($handler->handle($request->withUri($request->getUri()->withPath($target))));
    }
