...
final class RdataCanonicalizer
{
    ...
    private static function txt(string $value): string
    {
        ...
        foreach ($matches[1] as $match) {
            $decoded = preg_replace_callback('/\\\\([0-9]{3}|.)/', static function (array $m): string {
                return ctype_digit($m[1]) ? chr((int) $m[1]) : $m[1];
            }, $match[0]);
    ...
    private static function svcb(string $value): string
    {
        ...
        return (string) (int) $parts[0] . ' ' . strtolower(rtrim($parts[1], '.')) . (isset($parts[2]) ? ' ' . $parts[2] : '');
    }
